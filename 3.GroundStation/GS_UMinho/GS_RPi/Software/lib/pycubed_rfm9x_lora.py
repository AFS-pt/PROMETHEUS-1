...
class RFM9x:
    ...
    operation_mode = _RegisterBits(Constants._RH_RF95_REG_01_OP_MODE, bits=3)

    low_frequency_mode = _RegisterBits(Constants._RH_RF95_REG_01_OP_MODE, offset=3, bits=1)
    ...
    output_power = _RegisterBits(Constants._RH_RF95_REG_09_PA_CONFIG, bits=4)
    ...
    pa_dac = _RegisterBits(Constants._RH_RF95_REG_4D_PA_DAC, bits=3)

    dio0_mapping = _RegisterBits(Constants._RH_RF95_REG_40_DIO_MAPPING1, offset=6, bits=2)

    auto_agc = _RegisterBits(Constants._RH_RF95_REG_26_MODEM_CONFIG3, offset=2, bits=1)

    low_datarate_optimize = _RegisterBits(
        Constants._RH_RF95_REG_26_MODEM_CONFIG3, offset=3, bits=1
    )
    ...
    def __init__(
        ...
        *,
        ...
        if self.operation_mode != Constants.SLEEP_MODE or not self.long_range_mode:
        ...
        self._write_u8(Constants._RH_RF95_REG_0E_FIFO_TX_BASE_ADDR, 0x00)
        self._write_u8(Constants._RH_RF95_REG_0F_FIFO_RX_BASE_ADDR, 0x00)
        # Set mode idle
        self.idle()
        # Set frequency
        ...
        self.node = Constants._RH_BROADCAST_ADDRESS
        ...
        self.destination = Constants._RH_BROADCAST_ADDRESS
    ...
    def preamble_length(self) -> int:
        ...
        """
        msb = self._read_u8(Constants._RH_RF95_REG_20_PREAMBLE_MSB)
        lsb = self._read_u8(Constants._RH_RF95_REG_21_PREAMBLE_LSB)
        return ((msb << 8) | lsb) & 0xFFFF

    @preamble_length.setter
    def preamble_length(self, val: int) -> None:
        assert 0 <= val <= 65535
        self._write_u8(Constants._RH_RF95_REG_20_PREAMBLE_MSB, (val >> 8) & 0xFF)
        self._write_u8(Constants._RH_RF95_REG_21_PREAMBLE_LSB, val & 0xFF)
    ...
    def frequency_mhz(self, val: Literal[433.0, 915.0]) -> None:
        ...
        # Extract byte values and update Constants.
        msb = frf >> 16
        mid = (frf >> 8) & 0xFF
        lsb = frf & 0xFF
    ...
    def snr(self) -> float:
        ...
        snr_byte = self._read_u8(Constants._RH_RF95_REG_19_PKT_SNR_VALUE)
        if snr_byte > 127:
    ...
    def signal_bandwidth(self) -> int:
        ...
        bw_id = (self._read_u8(Constants._RH_RF95_REG_1D_MODEM_CONFIG1) & 0xF0) >> 4
    ...
    def signal_bandwidth(self, val: int) -> None:
        ...
        for bw_id, cutoff in enumerate(self.bw_bins):
            ...
            bw_id = 9
        self._write_u8(
            Constants._RH_RF95_REG_1D_MODEM_CONFIG1,
            (self._read_u8(Constants._RH_RF95_REG_1D_MODEM_CONFIG1) & 0x0F) | (bw_id << 4),
        )
        if val >= 500000:
    ...
    def coding_rate(self) -> Literal[5, 6, 7, 8]:
        ...
        cr_id = (self._read_u8(Constants._RH_RF95_REG_1D_MODEM_CONFIG1) & 0x0E) >> 1
        denominator = cr_id + 4
        return denominator
    ...
    def coding_rate(self, val: Literal[5, 6, 7, 8]) -> None:
        ...
        self._write_u8(
            Constants._RH_RF95_REG_1D_MODEM_CONFIG1,
            (self._read_u8(Constants._RH_RF95_REG_1D_MODEM_CONFIG1) & 0xF1) | (cr_id << 1),
        )
    ...
    def spreading_factor(self) -> Literal[6, 7, 8, 9, 10, 11, 12]:
        ...
        sf_id = (self._read_u8(Constants._RH_RF95_REG_1E_MODEM_CONFIG2) & 0xF0) >> 4
        return sf_id
    ...
    def spreading_factor(self, val: Literal[6, 7, 8, 9, 10, 11, 12]) -> None:
        ...
        self._write_u8(Constants._RH_RF95_DETECTION_THRESHOLD, 0x0C if val == 6 else 0x0A)
        self._write_u8(
            Constants._RH_RF95_REG_1E_MODEM_CONFIG2,
            (
                (self._read_u8(Constants._RH_RF95_REG_1E_MODEM_CONFIG2) & 0x0F)
                | ((val << 4) & 0xF0)
            ),
        )
    ...
    def enable_crc(self) -> bool:
        ...
        return (self._read_u8(Constants._RH_RF95_REG_1E_MODEM_CONFIG2) & 0x04) == 0x04
    ...
    def enable_crc(self, val: bool) -> None:
        ...
        if val:
            self._write_u8(
                Constants._RH_RF95_REG_1E_MODEM_CONFIG2,
                self._read_u8(Constants._RH_RF95_REG_1E_MODEM_CONFIG2) | 0x04,
            )
        else:
            self._write_u8(
                Constants._RH_RF95_REG_1E_MODEM_CONFIG2,
                self._read_u8(Constants._RH_RF95_REG_1E_MODEM_CONFIG2) & 0xFB,
            )

    def tx_done(self) -> bool:
        """Transmit status"""
        return (self._read_u8(Constants._RH_RF95_REG_12_IRQ_FLAGS) & 0x8) >> 3

    def rx_done(self) -> bool:
        """Receive status"""
        return (self._read_u8(Constants._RH_RF95_REG_12_IRQ_FLAGS) & 0x40) >> 6
